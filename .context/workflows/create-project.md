...
3. **MindGraph Scan (mandatory for planning):**
   ```bash
   bin/mindgraph doctor
   bin/mindgraph query "<topic keywords>" --db ~/.mindgraph/mainframe.sqlite --json --top-k 6
   bin/mindgraph query "<slug or topic>" --db ~/.mindgraph/mainframe-projects.sqlite --json --top-k 6
   ```
   Paste a filled **MindGraph Query Pass** into `log.md` (or the first plan) using
   [`.context/templates/mindgraph-query-pass.md`](../templates/mindgraph-query-pass.md).
   Keep knowledge vs project nominations labeled; do not blend rankings.
