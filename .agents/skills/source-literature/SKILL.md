...
2. Continue the full research pass via `.context/workflows/research-lane-loop.md` steps 4–7 (ingest → synthesize → close → loop decision), or at minimum:
   - Run ingest per `.context/workflows/ingest-minion.md`
   - `bin/post-route-enrich --subset <domain>` (set `UNPAYWALL_EMAIL` for Unpaywall)
   - Write or update the phase synthesis note when ≥3 related raws exist
   - Update the lane capture index + `log.md`
3. For Tier C/E or high-stakes claims, schedule `bin/audit-sweep` after routing.

This skill **stops at inbox**. For the end-to-end loop (source-literature through knowledge synthesis), invoke `research-lane-loop`.
