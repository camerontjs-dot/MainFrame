...
class GenerateProjectTasksTests(unittest.TestCase):
    def test_phase_tasks_and_compiled_packets_are_both_preserved(self) -> None:
        ...
        self.assertEqual(extracted_phase_task.get("authority_class"), "plan_item")
        self.assertFalse(extracted_phase_task.get("executable"))
        self.assertEqual(packet.get("authority_class"), "packet_candidate")
        self.assertFalse(packet.get("executable"))

        envelope = generate_tasks.build_quarantine_envelope(tasks)
        self.assertEqual(envelope["quarantine"]["status"], "quarantined")
        self.assertTrue(generate_tasks.is_quarantined_manifest(envelope))
